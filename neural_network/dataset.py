...
def collate_fn_padd(batch):
    ...
    label_lengths = []
    for item in batch:
        # spectrogram, label, input_length, label_length = item
        spectrogram = item[0] if item is not None else None
        label = item[1] if item is not None else None
        input_length = item[2] if item is not None else None
        label_length = item[3] if item is not None else None
    ...
    if len(spectrograms) == 0:  # If all items were None
